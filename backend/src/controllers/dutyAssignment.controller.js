...
exports.generateAssignments = async (req, res) => {
    try {
        ...
        const sortedUsers = allUsers.sort((a, b) => {
            const scoreA = (a.experienceYears || 0) + (designationScores[a.designation] || 0);
            const scoreB = (b.experienceYears || 0) + (designationScores[b.designation] || 0);
        ...
        const assignUser = (roleName, locationId, locationType) => {
            ...
            const assignedUser = sortedUsers.find(user => !assignedUserIds.has(user._id.toString()));
            if (assignedUser) {
        ...
        blocks.forEach(block => assignUser("Block Supervisors", block._id, "assignedBlockId"));

        // --- Assign Random Roles (Invigilators) ---
        const rooms = await Room.find({ sectionId: await Section.find({ blockId: blocks.map(b => b._id) }).then(s => s.map(sec => sec._id)) });
        const unassignedInvigilatorPool = allUsers.filter(user => !assignedUserIds.has(user._id.toString()));
        const shuffledInvigilators = unassignedInvigilatorPool.sort(() => 0.5 - Math.random());
        ...
        const supportRoles = [
            "Pre-Test Coordination Team", "Post-Test Coordination Team", "ICT Staff", 
            "Transportation Staff", "Security Staff", "Health Staff (Medical/First Aid)", 
            "Marketing Staff (Branding, Communication)", "Engineering Staff (Power, Equipment)", 
            "Janitorial Staff (Cleanliness)"
        ];
        
        const unassignedStaffPool = allUsers.filter(user => !assignedUserIds.has(user._id.toString()));
